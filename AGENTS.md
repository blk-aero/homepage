...
- Do not read `ARCHIVE.md` or `/_Archive` unless explicitly asked.
...
- `npm run build` runs `astro build` plus the opt-in IndexNow `postbuild` hook; without `INDEXNOW_KEY`, it should skip external submission and exit 0.
...
- [Design and UI guidance](docs/agents/design-ui.md)
