...
describe("project scripts", () => {
  it("defines required scripts", () => {
    ...
    expect(pkg.scripts).toMatchObject({
      ...
      "test:built": "npm run build && vitest run tests/config/crawlable-built-html.test.ts tests/config/public-runtime-guard.test.ts tests/config/deployment-release.test.ts",
